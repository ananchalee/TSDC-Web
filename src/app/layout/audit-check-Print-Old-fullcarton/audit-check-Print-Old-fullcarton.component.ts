...
@Component({
  selector: 'app-audit-check-Print-Old-fullcarton',
  templateUrl: './audit-check-Print-Old-fullcarton.component.html',
  styleUrls: ['./audit-check-Print-Old-fullcarton.component.scss']
})
export class AuditCheckPrintOldFullComponent implements OnInit {
  busy!: Subscription;
  ...
  isLoading = false;
  isCoverSheetRunning = false; //// กันกดปุ่มออกใบปะซ้ำ (double click)
...
console.log(this.input)
              ...
              this.input.p = this.sumqty[0].USER_DEF5.substring(2, 3);
                ...
                Swal.fire({
                  ...
                  if (result.value) {
                    ...
                    setTimeout(() => { this.focusInput_item() }, 1000)
                  } else {
                    this.input.CONTAINER_ID = ''
                  }
                })
  ...
  updateQTY() {
    console.log("UPDATE");
    if (this.input.QTY == null || this.input.QTY > this.input.QTY_REQUESTED - this.input.QTY_CHECK || this.input.QTY <= 0) {
      this.playAudioError();
      Swal.fire({
        icon: 'warning',
      ...
      this.dataService.updateConQtyCheck_Old_full(this.input).subscribe(res => {
        var data: any = res
        if (data.status === 'error') {
          Swal.fire({
            icon: 'error',
